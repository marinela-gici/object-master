...
const dividedByThree = pokémon.filter(item => item.id % 3 === 0);
...
const greaterId = pokémon.filter(item => item.id > 99).map(item => item.name);
console.log(greaterId);

// an array with just the names of the pokémon whose only type is poison
const poisonType = pokémon.filter(item => item.types.includes('poison') && item.types.length === 1).map(item => item.name);
console.log(poisonType);

// an array containing just the first type of all the pokémon whose second type is "flying"
const flyingType = pokémon.filter(item => item.types[1] === 'flying').map(item => item.types[0]);
